<template>
    ...
    <div class="bg-black top-10 text-lg pt-10 w-full h-full">
        ...
        <span class="text-xl font-semibold ml-2 " ></span>

        <GoogleMapp :tourInfo="toursInfo"/>
        <div class="h-20" ></div>
        
    </div>
</template>
<script>
    import GoogleMapp from '../../components/GoogleMapp.vue'
    ...
    setup() {
        const { toursInfo, loading, error }  = storeToRefs(useTourStore())
        const { fetchTourbyId }             = useTourStore()
        fetchTourbyId(1)
        return { toursInfo , loading , error }
        }
}
</script>